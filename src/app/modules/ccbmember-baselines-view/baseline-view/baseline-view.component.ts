      ...
      grouped.forEach((groupNodes, groupTitle) => {
        ...
        groupNodes.forEach((node) => {
          const treeNode: any = {
            ...
            icon: node.fontIcon,
            property:
            [TypeWorkpackEnumWBS.Deliverable, TypeWorkpackEnumWBS.Milestone]
              .includes(node.type as TypeWorkpackEnumWBS) ? 'value' :
            (node.type as TypeWorkpackEnumWBS) === TypeWorkpackEnumWBS.Organizer ? 'subtitle' : 'title',
            expanded: this.treeShouldStartExpanded,
            children: [],
