...
export class WorkpackSectionDashboardComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  constructor(
    ...
    this.responsiveSrv.resizeEvent.subscribe((currentValue) => {
      ...
      this.windowProperties = {
        isMenuPanelOpen:
          currentValue.width <= this.windowProperties.currentWindowSize,
      ...
      .subscribe(() =>
        setTimeout(() => {
          if (this.calendarComponent) {
            ...
            this.calendarComponent.dateFormat = this.translateSrv.instant(
              'dateFormatMonthYear'
            );
            this.calendarComponent.updateInputfield();
          }
          this.midleTextMilestones = this.translateSrv.instant(
            'milestonesLabelChart'
          );
          this.midleTextRisks = this.translateSrv.instant('risksLabelChart');
          this.midleTextDelivable = this.translateSrv.instant(
            'DeliverableLabelChart'
          );
    ...
    this.dashboardSrv.observableResetDashboard
      .pipe(takeUntil(this.$destroy))
      .subscribe((reset) => {
        if (reset) {
  ...
  async ngOnChanges(changes: SimpleChanges) {
    if (changes.collapsePanelsStatus) {
  ...
  loadDashboardData() {
    ...
    if (this.dashboard && this.dashboard.workpacksByModel) {
      ...
      cards.forEach((card) => {
        const cardModelIndex = this.dashboard.workpacksByModel.findIndex(
          (c) =>
            c.idWorkpackModel === card.idWorkpackModel && c.level < card.level
        );
        if (cardModelIndex > -1) {
          ...
          const cardIndex = this.dashboard.workpacksByModel.findIndex(
            (c) => c === card
          );
  ...
  async getDashboard() {
    this.isLoading = true;
    this.dashboardSrv.getDashboard({
  ...
  closeItemsMenu(index) {
    this.dashboard.workpacksByModel[index].menuItems =
      this.dashboard.workpacksByModel[index].menuItems &&
      this.dashboard.workpacksByModel[index].menuItems.map((item) => ({
        ...item,
        expanded: false,
      }));
  }
