...
import {
  ...
  IScheduleDetail,
  IStep,
} from 'src/app/shared/interfaces/ISchedule';
...
export class StepComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.translateSrv.onLangChange
      ...
      .subscribe(({ lang }) => {
        ...
        setTimeout(() =>
          this.calendarComponents?.map((calendar) => {
            calendar.ngOnInit();
    ...
    if (this.idStep) {
      ...
      if (result.success) {
        ...
        this.step = {
          ...
          consumes:
            this.stepDetail.consumes &&
            this.stepDetail.consumes.map((consume) => ({
  ...
  async setBreadcrumb() {
    ...
    this.breadcrumbSrv.setMenu([
      ...breadcrumbItems,
      {
        ...
        info: !this.idStep
          ? ''
          : moment(this.stepDetail.periodFromStart).format('MMMM'),
      },
    ]);
  }
  ...
  async loadPermissions() {
    ...
    if (result.success) {
      if (isUserAdmin) {
        ...
        this.editPermission =
          result.data.permissions.filter((p) => p.level === 'EDIT').length >
            0 && !result.data.canceled;
      }
      if (
        result.data.endManagementDate &&
        result.data.endManagementDate !== null
      ) {
  ...
  setStepFormValues() {
    switch (this.stepType) {
      case 'start':
        ...
        if (this.stepDetail) {
          this.formStep.reset({
            ...
            end: this.onlyOneStep ? new Date(this.end) : null,
            plannedWork: this.stepDetail.plannedWork,
            actualWork: this.stepDetail.actualWork,
          });
        }
      ...
      case 'end':
        ...
        if (this.stepDetail) {
          this.formStep.reset({
            start: this.onlyOneStep ? new Date(this.start) : null,
  ...
  loadCostAssignmentSession() {
    if (
      !this.menuItemsCostAccounts ||
      this.menuItemsCostAccounts.length === 0
    ) {
      return;
    }
    ...
    if (this.step && this.step.consumes) {
      ...
      this.costAssignmentsCardItems = this.step.consumes.map((consume) => {
        const costAccount = this.costAccounts.find(
          (cost) => cost.id === consume.idCostAccount
        );
        const propertyModelName =
          costAccount && costAccount?.models.find((p) => p.name === 'name');
        const propertyName =
          costAccount && propertyModelName
            ? costAccount?.properties.find(
                (p) => p.idPropertyModel === propertyModelName.id
              ).value
            : consume.nameCostAccount;
        return {
          type: 'cost-card',
          unitMeasureName: '$',
          ...
          costAccountAllocation: costAccount?.costAccountAllocation && {
            ...
            planed:
              costAccount?.costAccountAllocation.planed - consume.plannedCost,
            actual:
              costAccount?.costAccountAllocation.actual - consume.actualCost,
          },
        };
      });
  ...
  async loadMenuItemsCostAccounts() {
    const costAccountsIds = this.step?.consumes
      ? this.step.consumes.map((consume) => consume.idCostAccount)
      : [];
    if (!this.costAccounts) {
      const result = await this.costAccountSrv.GetAll({
        'id-workpack': this.schedule.idWorkpack,
      });
      if (result.success) {
    ...
    const workpacksIdsNotRepeated = workpacksIds.filter(
      (w, i) => workpacksIds.indexOf(w) === i
    );

    this.menuItemsCostAccounts = workpacksIdsNotRepeated.map((idWorkpack) => {
      const workpackCostAccounts = this.costAccounts.filter(
        (cost) => cost.idWorkpack === idWorkpack
      );
      return {
        label: workpackCostAccounts[0].workpackModelName,
        items: workpackCostAccounts.map((workpackCost) => {
          const propertyModelName = workpackCost.models.find(
            (p) => p.name === 'name'
          );
          const propertyName = workpackCost.properties.find(
            (p) => p.idPropertyModel === propertyModelName.id
          );
          return {
            ...
            disabled:
              costAccountsIds.includes(workpackCost.id) || !this.editPermission,
            command: () =>
              this.createNewCardItemCost(
                workpackCost.id,
  ...
  createNewCardItemCost(idCost: number, costName: string) {
    ...
    const costAccountSelected = this.costAccounts.find(
      (cost) => cost.id === idCost
    );
    const costAssignmentsCardItemsList = this.costAssignmentsCardItems.filter(
      (card) => card.type === 'cost-card'
    );
    const costAccountsIds = costAssignmentsCardItemsList.map(
      (cardItem) => cardItem.idCost
    );
    this.menuItemsCostAccounts = Array.from(
      this.menuItemsCostAccounts.map((group) => {
        const items = group.items.map((item) => ({
          ...item,
          disabled:
            costAccountsIds.includes(Number(item.id)) ||
            Number(item.id) === idCost,
        }));
        return {
          ...group,
          items,
  ...
  deleteCost(idCost: number) {
    ...
    this.costAssignmentsCardItems = Array.from(
      this.costAssignmentsCardItems.filter((cost) => cost.idCost !== idCost)
    );
    this.reloadCostAssignmentTotals();
  ...
  reloadCostAssignmentTotals() {
    if (this.costAssignmentsCardItems) {
      const plannedTotal = this.costAssignmentsCardItems
        .filter((card) => card.type === 'cost-card')
        .reduce((total, cost) => total + cost.plannedWork, 0);
      const actualTotal = this.costAssignmentsCardItems
        .filter((card) => card.type === 'cost-card')
  ...
  async saveStep() {
    ...
    const start = moment(this.formStep.controls.start.value).format(
      'yyyy-MM-DD'
    );
    const hasNegativeValues =
      this.formStep.controls.plannedWork.value < 0 ||
      this.formStep.controls.actualWork.value < 0 ||
      (this.costAssignmentsCardItems &&
        this.costAssignmentsCardItems.filter(
          (item) =>
            item.type === 'cost-card' &&
            (item.plannedWork < 0 || item.actualWork < 0)
        ).length > 0);
    ...
    this.step = {
      ...
      endStep:
        this.stepType === 'end' || this.stepType === 'newEnd' ? true : false,
      scheduleEnd:
        this.stepType === 'end' || this.stepType === 'newEnd'
          ? end
          : this.onlyOneStep && this.step && this.step?.id
          ? end
          : null,
      scheduleStart:
        this.stepType === 'start' || this.stepType === 'newStart'
          ? start
          : this.onlyOneStep && this.step && this.step?.id
          ? start
          : null,
  ...
  handlePassedMonths() {
    ...
    if (
      stepDate.isBefore(startOfCurrentMonth) &&
      stepDate.isBefore(endOfPreviousMonth)
    ) {
