...
export class IndicatorComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.responseService.observable
  ...
  mirrorDescription(): boolean {
    return (
      isNaN(this.idIndicator) &&
      this.formIndicator?.get('description')?.pristine
    );
  }
  ...
  async loadFontOptions(idOffice: number) {
    return new Promise<void>((resolve) => {
      this.indicatorSrv
        .loadOrganizationFromOffice(idOffice)
        .subscribe((data: any) => {
          this.sourceOptions = data.data.map((item) => ({
            name: item,
          }));
          resolve();
        });
  ...
  generatePeriodData(startDate: Date, endDate: Date): void {
    ...
    this.periodList = this.formatPeriod(
      years,
        ...
        if (this.selectedPeriodicity === 'ANUAL') {
          this.indicator.periodGoals.forEach((goal) => {
            ...
            if (goal.lastUpdate && goal.lastUpdate !== '--') {
              if (
                !existingGoalsMap[year].lastUpdate ||
                new Date(goal.lastUpdate) >
                  new Date(existingGoalsMap[year].lastUpdate)
              ) {
        ...
        return {
          ...
          justification:
            this.selectedPeriodicity === 'ANUAL'
              ? null
              : existingGoal
              ? this.indicator.periodGoals?.find((g) => g.period === period)
                  ?.justification || null
              : null,
        };
      });
  ...
  async saveIndicator() {
    ...
    const result = put
      ? await this.indicatorSrv.put(sender)
      : await this.indicatorSrv.post(sender);
