...
export class CardComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Input() displayProgressBarIfTotalIsZero: boolean = false;
  ...
  constructor(
    ...
    this.workpackSrv.observableCanEditCheckCompleted
      .pipe(takeUntil(this.$destroy))
      .subscribe((canEdit) => {
        if (canEdit) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      ...
      this.showAnimationSearch =
        !this.properties.searchTerm || this.properties.searchTerm === ''
          ? false
          : true;

      if (
        !this.displayProgressBarIfTotalIsZero &&
        this.properties.progressBarValues
      ) {
        // Remove as barras de progresso caso o Total seja zero
        this.properties.progressBarValues =
  ...
  loadFilterListOptions() {
    ...
    if (this.properties && this.properties.filters) {
      this.properties.filters.forEach((filter) => {
        ...
        if (
          this.properties.idFilterSelected ||
          this.properties.idFilterSelected === null
        ) {
  ...
  handleSearchText() {
    const inputtext = document.getElementById(
      'id-app-inputtext' + this.properties.cardTitle
    );
    inputtext.focus();
  ...
  get label(): string {
    return this.properties.workpackType
      ? this.properties.workpackType === 'Milestone'
        ? 'completed'
        : 'scopeCompleted'
      : '';
  }
