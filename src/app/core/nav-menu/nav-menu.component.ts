...
export class NavMenuComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.menuSrv.hasFavoriteItems
      .pipe(takeUntil(this.$destroy))
      .subscribe((hasFavoriteItems) => {
  ...
  selectMenuActive(url: string) {
    ...
    if (!url.startsWith('reports')) {
      const reportButton = this.menus.find(
        (btn) => btn.label === MenuButtons.REPORTS
      );
    ...
    if (!url.startsWith('search')) {
      const searchButton = this.menus.find(
        (btn) => btn.label === MenuButtons.UNIVERSAL_SEARCH
      );
  ...
  async loadReportsMenu() {
    const result = await this.reportSrv.checkHasActiveReports({
